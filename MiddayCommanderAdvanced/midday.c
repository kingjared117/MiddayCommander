...
int main(){
	...
	while(1){
	 ...
	 if(wait3(NULL,WNOHANG,NULL)>0){
	  ...
	  (waitusage->ru_utime.tv_sec*1000+waitusage->ru_utime.tv_usec/1000), waitusage->ru_majflt, waitusage->ru_minflt);
	  //for(i=0;process[i].pid!=pid;i++);
	  //process[i].finish=1;
	 }
				...
				if(atoi(inCh)==0 && inCh[0]!='0')
					...
					switch(atoi(inCh)){
						...
						case 2:
							...
							break;
						default:
						  execFunc(funcs[atoi(inCh)]/*,process,backSize,currPID*/);
							break;
						}
				break;
			}
